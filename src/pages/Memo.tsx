...
import { sampleConcepts, sampleMemos } from "@/data/sampleConcepts";
...
const Memo = () => {
  const { id } = useParams();
  const concept = sampleConcepts.find((c) => c.id === id) ?? sampleConcepts.find((c) => c.id === "c-002")!;
  const memo = sampleMemos[concept.id] ?? sampleMemos["c-002"];
            ...
            <div className="mt-5 flex flex-wrap items-start justify-between gap-6">
              <div className="max-w-3xl">
                <div className="mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
                  Memo · {concept.id} · {concept.vertical}
                </div>
        ...
        <article className="container max-w-5xl py-14">
          ...
          <Section eyebrow="02" title="Market attractiveness">
            <div className="flex items-center gap-3 mb-1">
              <BandPill value={memo.marketAttractiveness.band} label={`market: ${memo.marketAttractiveness.band}`} />
            </div>
          ...
          <Section eyebrow="03" title="Competitive crowding">
            <div className="flex items-center gap-3 mb-1">
              <BandPill value={memo.competitiveCrowding.band} label={`crowding: ${memo.competitiveCrowding.band}`} />
            </div>
            ...
            <div className="rounded-lg border border-border overflow-hidden mt-4">
              ...
              <ul className="divide-y divide-border">
                {memo.competitors.map((c) => (
                  <li key={c.name} className="grid grid-cols-1 md:grid-cols-[1.2fr_1fr_1.2fr_1.6fr] gap-3 px-4 py-3 text-sm">
          ...
          <Section eyebrow="04" title="Market sizing">
            ...
            <div className="text-xs text-muted-foreground italic">
              Sizing confidence: <span className="capitalize text-foreground">{memo.marketSizing.confidence}</span>. Ranges, not point estimates.
            </div>
          </Section>
          ...
          <Section eyebrow="06" title="Recommendation">
            <div className="rounded-xl border border-border bg-gradient-to-br from-primary-soft to-card p-6 shadow-sm">
              ...
              <h3 className="font-display text-2xl font-semibold tracking-tight mt-4">
                Move to validation with a 2-week design-partner probe.
              </h3>
              <div className="mt-5">
                <div className="mono text-[10px] uppercase tracking-widest text-muted-foreground mb-2">
                  Proposed next steps
          ...
          <Section eyebrow="07" title="Sources & verification">
            <ul className="space-y-3">
              {memo.sources.map((s) => (
                <li key={s.url} className="rounded-lg border border-border bg-card p-4 flex items-start justify-between gap-4">
                  <div>
                    <a href={s.url} target="_blank" rel="noreferrer" className="font-medium text-foreground hover:text-primary transition-colors">
                      {s.title}
                    </a>
